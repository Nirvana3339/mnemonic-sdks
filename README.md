...
Install Mnemonic as an MCP server so Claude Desktop or Cursor can recall and capture lessons automatically — no code changes to your agent.

```bash
npm install @mnemonic-ai/mcp
```
...
Restart Claude Desktop. You'll see two new tools available:

| Tool | When to use |
|------|-------------|
| `mnemonic_recall` | Before any non-trivial task — fetches relevant lessons |
| `mnemonic_capture` | After every task — records outcome so Mnemonic learns |
...
| `MNEMONIC_AGENT_ID` | ✅ | `mcp-agent` | Agent identifier |
| `MNEMONIC_TOP_K` | ❌ | `5` | Max lessons to retrieve |
| `MNEMONIC_BASE_URL` | ❌ | production URL | Override API endpoint |
...
mnemonic-sdks/
├── python/                     ← pip install mnemonic-sdk
├── javascript/                 ← npm install @mnemonic-ai/sdk
└── mcp/                        ← npx @mnemonic-ai/mcp
    └── src/index.ts            # MCP server (recall + capture tools)
```
